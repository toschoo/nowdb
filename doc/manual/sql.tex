...
clauses.
In all clauses that allow expressions,
a constant value is a valid expression,
\eg\ $1, +4, 3.14159,$ \etc\
...
If $\circ$ is binary operator
and $a$ and $b$ are valid expressions,
then
$a \circ b$
is a valid expression too.
...
by functions. Functions are very
similar to operators, but differ
in the syntax. Functions have the
form
...
expects arguments of certain types.
...
Promotion for numerical types usually follows 
...
Likewise, it at least one of the operands
...
The fourth operator (\%) is the remainder of
the Euclidean division.
It operates only on integral values and results
in a type error if applied to a \term{float}.
...
But that is not the case. The remainder the
...
then then quotient is positive and 
...
\paragraph{Abs and Rounding}
The function $abs$ computes the
absolute value of argument, \eg:
$abs(-1)$ is 1.
...
otherwise, the next integer; \eg: $ceil(1.1)$ is 2.
...
In particular, when converting a \term{float}
...
\comment{Booleans are currently only available in
the \term{where} clause}

Boolean operators and functions
are those that result in a Boolean value.
...
= < > <= >= != <>
\end{verbatim}
which translate to the mathematical operations
$=,<,>,\le,\ge,\neq$,
where for $\neq$ two ways of writing are
permitted, namely
\begin{verbatim} 
!= <>
\end{verbatim}
...
there type is equal (or compatible
...
\comment{Available right now is \keyword{in}.
\keyword{is} and \keyword{between} will be
available soon. Since \keyword{exists},
\keyword{any} and \keyword{all} are useful
only with subqueries, it will take some
more time until they are available.}
...
since in this form the first
element of the range is excluded.
Correspondingly,
\keyword{between} $[1,2[$
would exclude the last element of the range
and
\keyword{between} $]1,2[$
would exclude both.
...
This is also true for the operators
...
\comment{
The operators \keyword{any} and \keyword{all}
are still to be defined.
}

\subsubsection{Bitwise Operators}
shiftl, shiftr, xor, \&, |, bit
...
now, dusk, dawn 
\comment{Not yet available.
The time component functions (year, month, \etc)
are extremely interesting.
At the moment it is hard to select sub periods;
for instance: for the years 2000-2010, show me
the average temperature of month June:
}
...
\keyword{select} \keyword{count}($\ast$) \keyword{from} \identifier{sales}
...
is the number of rows (\ie\ \keyword{count}($\ast$)).
...
\keyword{select} \identifier{integral}(\keyword{weight})
\keyword{from} \identifier{sales}
...
\keyword{select} \identifier{count}(*) \\
...
for instance,
\keyword{where} \keyword{true} or
\keyword{where} \identifier{year}(\keyword{stamp}) = 1984.
This will change soon, so that all expressions
will be available in the \term{where} clause.}
