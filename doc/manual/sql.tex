...
and text. Keywords and identifiers are mutually
...
that is felt in its impact, but remains (mostly) invisible.
In the future types (which, then, will be called tables),
will be stored per attribute (a.k.a. column)
and edges will be stored in edgestores
(which are tables with a fixed structure).
With this
all the delicateness concerning
...
They also may be functions, constants or complex expressions.
...
Those are recursive combinations
of fields, constants, functions and operators
that can be nested together.
...
$(a+b)/n, (weight-w)^2$ and so on.
On the long run, we need to address expressions
in projection and \term{where} clause, but also in
updates and inserts.
...
\keyword{select} 'buys', \keyword{origin},
                         ...
                         \keyword{weight},
                         \keyword{weight2} \\
\hspace*{0.2cm}\keyword{from} \identifier{sales}

It is possible to define an alias for the data source:

\keyword{from} \identifier{sales} \keyword{as} \identifier{s}

The table \identifier{sales} can now be referred to
in the \term{select} clause as ``s''.
...
\comment{I am very proud of this wording.
It describes clearly what a join is
and it makes the difference
with relational databases very clear:
there is not way to combine vertices
other than by using an edge.
This said, I am not sure whether other
kinds of joins will be needed in the future.
...
\keyword{join} \identifier{mytype} \keyword{on}
               (\keyword{edgefield}, \identifier{typefield})

To make this more concrete:

\keyword{from} \identifier{sales} 
\keyword{join} \identifier{client} \keyword{on} 
               (\keyword{origin}, \identifier{client\_key})

This would produce an \term{inner join} 
between \identifier{sales} and \identifier{mytype}.
...
joins in \nowdb\ are \term{abelian} and, indeed,
the order of edge and type is irrelevant.

Since the primary key of a type is known
and there is always exactly one field,
which is primary key, it is possible to simplify:
...
But these entities may have fields with the same name,
\eg\ we could have named the primary key in both,
...
and, as this section show, the design is already quite advanced.
...
I am therefore now looking for alternatives.
The most promising appears to be the \term{hash join} technique,
which is applied in many databases.
Two avoid wasting weeks of programming
I prefer to use a higher-level programming language like Python.
There is a technique (described in \ref{chpt_pythonemb})
called \term{functional cursors} that I want to use for that purpose.
To cut a long story short: joins are not yet available.
There are workarounds, but, indeed, we will need joins \acronym{asap}.
}

\subsection{Correlation} 
A correlation combines two or more edges.
\comment{Not yet available}
...
\keyword{and} (\keyword{timestamp} $<$ '2018-04-01'
...
In the simpleplest from a statement with
...
The group above would produce the following set
...
for performance considerations in general, however, please
refer to chapter \ref{chpt_opt}.

The real power of grouping, however, becomes evident
...
are applied on the the whole result set.
...
result. Those are values correspond
to a any of the rows in the result set.
It may be the first or the last or any other.
That is unspecified.
