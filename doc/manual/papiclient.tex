...
A basic usage example for \term{execute} is
...
An example involving Pandas is:
...
must be set explicitly set \emph{before} \term{execute}
is called. For this purpose Cursor provides the
method \term{setRowFormat} which accepts on argument,
...
where \term{field name} is the chosen name of the $nth$ field
