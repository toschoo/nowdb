...
The choice of the keyword \term{schema} is intended to 
...
For `normal' database tasks like \acronym{dba}
...
Let's have a look at the module \term{sales}
...
Notice first that we import the module \term{nowdb}.
...
In the \term{except}-part, we just create
...
Python garbage collector (\acronym{gc}).
...
its rows using a \term{with} statement.
...
But, of course, we can call \term{exec}
