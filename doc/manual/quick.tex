...
This sets the \term{home} directory, \ie\
the directory where daemon will find resources
like the Lua \nowdb\ script.
It is here set to \tech{/usr/local},
which means that the daemon will find
the script \tech{nowdb.lua} in 
the directory \tech{/usr/local/lua}.
...
to \term{stamp}). This field does not need to be mentioned
...
for \nowdb\ (which can be downloaded from \comment{xxx}).
This tool comes with a much larger feature set
and is described in \comment{another document}.
...
but instead of the edge \term{buys},
we use a type:
...
C, \CC, Go, Python (both, 2.7 and 3.5) and Lua (5.3).
...
of the \term{for} statement
frees the cursor resources.
Strictly speaking, the \term{with} statement
around the \term{execute} is not really necessary.
The \term{for} statement already handles
...
stored functions is Lua (version 5.3 or higher).
...
polymorphic \term{result type} that
...
can be added interactively through \acronym{sql}.
...
present in the global table \tech{nowdb}.
All functions and constants defined in
this table can be used just like the functionality
in the Lua standard packages (\term{string},
\term{table}, \term{os}, \etc).
\clearpage

The function revenue starts with
the definition of two local variables
\term{td} (for ``today'') and \term{tm} (for ``tomorrow'').
The variable \term{td} is derived from the parameter
by means of the \term{nowdb.round} function, which
takes two parameters: the first is a \term{time},
the second is a time unit to which we want to round.
The \term{nowdb} package defines several such units,
namely second, minute, hour and day.
The \term{round} function provides a simple means
...
directly with the \acronym{sql} time functions
that actually are aware of more subtle implications
of the notion of time.

The variable \term{tm} is then derived from \term{td}
by just adding one more day.

Both variables are used in the following
\acronym{sql} statement, which sums up the prices
paid ``today'' grouped by clients (\term{origin}).

Then we create an array with six elements, the local
variable \term{mx}, and, by means of the \term{nowdb.execute}
function, the cursor \term{cur}, which holds the result
of the query.

The function \term{execute} behaves similar to
the \term{rexecute} function we saw in the Python client.
...
of the \term{nowdb.error} function, which expects an
...
Finally, \term{revenue} returns the result
of a call to \term{nowdb.array2row}.
This function expects two arguments:
a list of \nowdb\ types and an array with
values corresponding to these types.
The types are \term{uints}, representing the
client key values in the \term{mx} array,
and \term{floats}, representing the revenue
for this day for this client.

\term{nowdb.array2row} creates a row result
using the array and the types. Internally,
it calls the \term{nowdb.makerow} function
and, for each element in the array, it calls
the method \term{add2row} provided for row results.
This method expects two parameters:
a \nowdb\ type and a corresponding value.
Finally, it closes the result by calling
the \term{closerow} method.

When we call \term{revenue} from the client like
...
of all global variables (\ie\ Lua's \term{global table})
...
variables \term{\_fibn1} and \term{\_fibn2},
which are initialised to 0 and 1 respectively.
The function \term{fibreset}, which takes no
parameters, resets the variables to these
initial values.

The function \term{fib}, which takes no parameters either,
computes the next Fibonacci number as
\term{\_fibn1} $+$ \term{\_fibn2} and
``shifts'' the module-wide variables one step
up in the Fibonacci sequence, namely to
the values \term{\_fibn2}, which is now
the new value for \term{\_fibn1}, and \term{f},
which is the new value for \term{\_fibn2}.
Finally, it returns the row result created by
the convenience function \term{nowdb.makeresult},
which creates a row with a single value.

When we create the interfaces for these procedures,
we can call \term{fib} like this:
...
\cmdline{
echo "exec fib(); exec fib(); exec fib(); $\backslash$\\
\hspace*{1cm} exec fib(); exec fib(); exec fib();" |$\backslash$\\
...
echo "exec fib(); exec fib(); exec fib(); $\backslash$\\
\hspace*{1cm} exec fib(); exec fib(); exec fib(); $\backslash$\\
\hspace*{1cm} exec fibreset(); $\backslash$\\
\hspace*{1cm} exec fib(); exec fib(); exec fib();" |$\backslash$\\
nowclient -q -d retail
}
...
is discussed in \comment{another document}.
