...
int insertEdges(char *e, uint32_t size) {
	int rc;
	uint32_t sz;
	int rm;
...
char *createBuf(uint32_t size) {
	char *buf;

	buf = calloc(1, size);
	if (buf == NULL) return NULL;

	if (insertEdges(buf, size) != 0) {
...
int checkSorted(char *e, int size) {
	char *p=NULL;
	int sz, rm;
	int cnt, exp;
	int b=0;
	...
	for(int i=0; i<size;) {
		...
		*/
		...
		// we have not written this value
		if (*(uint64_t*)(p+NOWDB_OFF_ORIGIN) == 0 ||
		    *(uint64_t*)(p+NOWDB_OFF_DESTIN) == 0 ||
		    *(int64_t*)(p+NOWDB_OFF_STAMP)   == 0) {
			fprintf(stderr, "invalid value in %d. buffer\n", b);
			return -1;
		}
	...
	if (sd > 0) {
		fprintf(stderr, "NOT SORTED for %u buffers: %d stepdowns\n",
		                                             size/BUFSZ, sd);
		return -1;
	}
	if (cnt != exp) {
...
int main() {
	...
	for(int i=0; i<10; i++) {
		int k = 1<<i;
		int sz = k*BUFSZ;
		fprintf(stderr, "testing with %d bytes\n", sz);
		if (testSort(sz) != 0) {
